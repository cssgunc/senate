...
def create_action_payload():
    return {"action_date": "2026-04-08", "description": "First reading", "action_type": "reading"}
...
def test_create_legislation(client, seeded_admins):
    ...
    res = client.post(
        "/api/admin/legislation", json=payload, headers={"Authorization": f"Bearer {token}"}
    )
...
def test_add_legislation_action(client, seeded_admins):
    ...
    leg_res = client.post(
        "/api/admin/legislation", json=leg_payload, headers={"Authorization": f"Bearer {token}"}
    )
    ...
    act_res = client.post(
        f"/api/admin/legislation/{leg_id}/actions",
        json=action_payload,
...
def test_update_legislation(client, seeded_admins):
    ...
    res = client.put(
        f"/api/admin/legislation/{leg_id}",
        json=update_payload,
        headers={"Authorization": f"Bearer {token}"},
    )
    assert res.status_code == 200
...
def test_update_legislation_action(client, seeded_admins):
    ...
    res = client.put(
        f"/api/admin/legislation/{leg_id}/actions/{action_id}",
        json=update_payload,
        headers={"Authorization": f"Bearer {token}"},
    )
    assert res.status_code == 200
...
def test_delete_legislation_and_cascade_actions(client, seeded_admins):
    ...
    # Delete legislation
    del_res = client.delete(
        f"/api/admin/legislation/{leg_id}", headers={"Authorization": f"Bearer {token}"}
    )
    assert del_res.status_code == 204
