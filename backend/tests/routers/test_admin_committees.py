def test_create_committee(client, test_db):
...
def test_update_committee(client, seeded_committees):
...
def test_delete_committee(client, test_db, seeded_committees):
    committee_to_delete = seeded_committees["committees"][1]  # Education Committee

    # Delete
    response = client.delete(f"/api/admin/committees/{committee_to_delete.id}")
    assert response.status_code == 204

    # Verify
    get_response = client.get(f"/api/committees/{committee_to_delete.id}")
    assert get_response.status_code == 404


def test_add_committee_member(client, seeded_committees):
...
def test_remove_committee_member(client, test_db, seeded_committees):
    ...
    response = client.delete(f"/api/admin/committees/{committee_id}/members/{senator_id}")
    assert response.status_code == 204

    # Verify deletion
    response_verify = client.delete(f"/api/admin/committees/{committee_id}/members/{senator_id}")
