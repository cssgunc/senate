...
export default function LegislationDetailsPage() {
  ...
  const handleAddAction = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await createLegislationAction({
        legislation_id: numId,
        action_date: new Date(actionDate).toISOString(),
        action_type: actionType,
        description: actionDescription,
      });
      // Clear form
      setActionDate(""); setActionType(""); setActionDescription("");
      // Refresh timeline
      fetchDetails();
    } catch (err) {
  ...
  const handleDeleteAction = async (actionId: number) => {
    ...
    try {
      await deleteLegislationAction(actionId);
      fetchDetails();
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <button onClick={() => router.back()} className="text-blue-600">&larr; Back to List</button>
      
      <div className="bg-white p-6 border rounded-lg shadow-sm">
        <h1 className="text-2xl font-bold mb-4">{legislation.bill_number}: {legislation.title}</h1>
        <div className="grid grid-cols-2 gap-4 text-sm text-gray-700">
          <p><strong>Status:</strong> {legislation.status}</p>
          <p><strong>Type:</strong> {legislation.type}</p>
          <p><strong>Sponsor:</strong> {legislation.sponsor_name}</p>
          <p><strong>Session:</strong> {legislation.session_number}</p>
        </div>
      </div>
              ...
              {legislation.actions.map(action => (
                <div key={action.id} className="border-l-4 border-blue-500 pl-4 py-1">
                  <div className="flex justify-between">
                    <strong>{new Date(action.action_date).toLocaleDateString()} - {action.action_type}</strong>
                    <button onClick={() => handleDeleteAction(action.id)} className="text-red-500 text-xs hover:underline">Delete</button>
                  </div>
        ...
        <div className="bg-white p-6 border rounded-lg shadow-sm h-fit">
          ...
          <form onSubmit={handleAddAction} className="space-y-4">
            ...
            <div>
              ...
              <textarea required className="w-full p-2 border rounded" rows={3} value={actionDescription} onChange={e => setActionDescription(e.target.value)} />
            </div>
            <button type="submit" className="w-full bg-blue-600 text-white rounded p-2 hover:bg-blue-700">Add Action</button>
          </form>
        </div>
      </div>
    </div>
